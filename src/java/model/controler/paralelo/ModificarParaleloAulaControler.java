...
import model.pojo.extend.listaParaleloIdParalelo;
@Controller
@RequestMapping("modificarparaleloaula.htm")
public class ModificarParaleloAulaControler {
    ...
    */
    ...
    {   //try {
        ...
        ModelAndView mav = null;
            ...
            paralelo.setIdparalelo(idparalelo);
            
            try {
                asigna = listaParaleloIdParalelo.obtenAsignacionIdParaleloDAO(paralelo);
                
            } catch (PersistentException ex) {
                Logger.getLogger(ModificarParaleloAulaControler.class.getName()).log(Level.SEVERE, null, ex);
            }
        if (!(asigna==null) ) {     
            mav = new ModelAndView("modificarparaleloaula");
            mav.addObject(asigna);
        System.out.println("valor:"+asigna.getAula().getIdaula());
        } else {
            mav = new ModelAndView ("redirect:dialogo.html");        
        }
        return mav;
    }
	...
	Asigna as) {
        ...
        as.setAnno(as.getAnno());
        as.setHoraentrada(as.getHoraentrada());
        as.setHorasalida(as.getHorasalida());
        model.pojo.extend.listaParaleloIdParalelo.update(as);
